...
_styles: >
  ...
  table.distance-metrics th:nth-child(1),
  table.distance-metrics td:nth-child(1) {
    width: 18%;
  }
  table.distance-metrics th:nth-child(2),
  table.distance-metrics td:nth-child(2) {
    width: 28%;
  }
  table.distance-metrics th:nth-child(3),
  table.distance-metrics td:nth-child(3) {
    width: 54%;
  }
...
<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig1b_length_strategy_generalization_lift.png' | relative_url }}" style="width:90%; margin-bottom: 10px" alt="Figure 1: Length and strategy generalization lift">
</center>
...
<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig1a_rlm_trajectory_isomorphism.png' | relative_url }}" style="width:80%; margin-bottom: 10px" alt="Figure 2: RLM trajectory isomorphism across BrowseComp-Plus and OOLONG">
</center>
...
We argued before, via the [**Mismanaged Geniuses Hypothesis (MGH)**](https://alexzhang13.github.io/blog/2026/mgh/) <d-cite key="zhang2026mgh"></d-cite>, that tasks which humans care about solving, can apparently almost always be decomposed rather naturally into sub-tasks that are not only far simpler but also which are not far beyond the current generation of language models. Compositional generalization is key for leveraging this in practice and enabling our systems’ reachable task space to extend beyond their training set’s direct coverage, especially for tasks that may look nothing alike in their surface-level tokens while sharing some underlying structure.

<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig2_mgh_long_task_decomposition.png' | relative_url }}" style="width:90%; margin-bottom: 10px" alt="Figure 3: MGH long-task decomposition">
</center>
...
<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig3_locally_in_distribution.png' | relative_url }}" style="width:100%; margin-bottom: 10px" alt="Figure 4: Locally in-distribution LM systems">
</center>
...
<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig4_context_offloading_programmatic_subcalls.png' | relative_url }}" style="width:100%; margin-bottom: 10px" alt="Figure 5: Context offloading and programmatic sub-agent calling">
</center>
...
The Recursive Language Model (RLM) harness is designed around abstracting tasks by their decomposition and deferring input-specific information to sub-calls. In this sense, RLMs view similarly decomposed problems as isomorphic<d-footnote>Notably, the argument below holds recursively, as we can treat each offloaded sub-agent as its own instance with its own “main” context. This is the intended design of an RLM being recursive, but it is not strictly necessary for maintaining locally in-distribution observations.</d-footnote>, and it achieves this by:
...
- [**OOLONG-Pairs**](https://www.alphaxiv.org/abs/2512.24601) (8k to 32k input, 7k to 146k output setting). A modified variant of OOLONG to find pairs of elements in the OOLONG dataset satisfying certain constraints.
...
<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig5_length_generalization_curves.png' | relative_url }}" style="width:100%; margin-bottom: 10px" alt="Figure 6: Length generalization training curves">
</center>
...
<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig6_strategy_generalization_curves.png' | relative_url }}" style="width:100%; margin-bottom: 10px" alt="Figure 7: Strategy generalization training curves">
</center>
...
<figure>
<center>
    <img src="{{ '/assets/img/lm_compo/fig7_trajectory_similarity.png' | relative_url }}" style="width:100%; margin-bottom: 10px" alt="Figure 8: Similarity of eval vs closest train trajectories">
</center>
...
Special thanks to Braden Hancock and the Laude Institute for generously providing several 8xH100 nodes on short notice to help run these experiments through the Laude Slingshots program. This research is supported by the NVIDIA Academic Grant Program. Alex thanks his wonderful labmates at MIT OASYS and Noah Ziems for helpful feedback while writing the blog.
